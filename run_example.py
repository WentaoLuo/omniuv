...
def gen_ts1():
    ts  =   86400*0.0 + np.arange(0, 24 * 60 * 60, 2, dtype = float)
    return ts
...
#    for i in range(28):
    for i in [0, 7, 14, 21]:
        _ts =   (i+0.5) * 86400 + np.arange(0, 3600, 2, dtype = float)
        ts.append(_ts)
...
# 2s AP, 0.5 hour per day, twice per day, 28 days
    ...
    for i in range(28):
#    for i in [0, 7, 14, 21]:
        _ts =   (i    ) * 86400 + np.arange(0, 900, 2, dtype = float)
        ts.append(_ts)
        _ts =   (i+0.5) * 86400 + np.arange(0, 900, 2, dtype = float)
        ts.append(_ts)
    ts  =   np.concatenate(ts)
    return ts
...
# Set to unity
    ...
    W       =   np.eye(3, dtype = float)
    task.set_eops(tmu = tmu, dut1 = dut1, PN = PN, W = W)
...
# Lunar, another fixed station, not included
    lf2   =   LunarFixed('M2')
    lf2.set_lfs(np.array([util.Rm, 0.0, 0.0])) 
    lf2.set_el_min_deg(15.0) 
    lf2.set_sep_min_deg(Earth=1.0, Moon=1.0)
    lf2.set_SEFD(2028)
#    stns.append(lf2)
...
# Sky frequency of each IF
    task.freqs  =   np.array([8.4E9])     # in Hz
 
# t_ap and bandwidth are only useful for calculating system 
# noise. See Eq. 5
...
# Usually the cell size is a fraction of the estimated resolution.
    ...
    res =   lam / rmax / 1E7 / np.pi * 180 * 3600E3 # in mas
